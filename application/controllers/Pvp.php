<?php class Pvp extends MY_Controller {
    ...
    public function pvplist(){
        ...
        redirect(site_url('pvp/viewlist'));
    }

    public function viewlist(){
        $this->data = $this->session->data;
        $this->load->view('pvp/pvplist');
    }
    ...
    public function create($player_id){
        // set this up to require 3 coins to attack a player
    ...
    public function hack(){
        ...
        if($opponent['hp'] <= 0){
            ...
            $opponent['message'] = 'You were HACKED by ' . $this->session->user['username'];
            $this->player_model->where('user_id', $event['opponent'])->update($opponent);
            // record the graffi wall message
            $string = $opponent['username'] . " was defeated by " . $this->session->username . " using " . $hack_name . ".";
        ...
        if($new_hp <=0 ){
            ...
            $this->system_message('You were killed by ' . $this->session->event['enemy_info']);
